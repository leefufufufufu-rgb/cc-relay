...
// The target is usually a native .exe (Windows) / binary (Unix), spawned directly.
// A .cmd/.bat wrapper needs a shell.
function spawnOpts(claude) {
  const useShell = process.platform === 'win32' && /\.(cmd|bat)$/i.test(claude);
  return { useShell };
}

function spawnInherit(claude, args, env) {
  const { useShell } = spawnOpts(claude);
...
function spawnCaptured(claude, args, env) {
  ...
  return new Promise((resolve) => {
    let out = '';
    const child = spawn(claude, args, { stdio: ['inherit', 'pipe', 'pipe'], env, shell: useShell });
